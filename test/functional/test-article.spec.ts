...
describe('Get /article/id', () => {
    ...
    it('Returns 404 for an invalid article', async () => {
        return agent.get('/article/00000').set('Accept', 'application/json').expect(404);
    });
    ...
    it('By default article is returned as JSON', async () => {
        await populateCollection('articles', [{ ...article }]);
        return agent.get('/articles/54296').expect('Content-Type', /json/).expect(200);
    });
    ...
    it('Can get article manifest', async () => {
        await populateCollection('articles', [
            {
                ...article,
                xml: '<article><body><fig id="fig1" position="float"><graphic xlink:href="elife-54296-fig1.tif" mimetype="image" mime-subtype="tiff"/></fig></body></article>',
            },
        ]);
        return agent
            ...
            .then((response) => {
                expect(response.body).toEqual({
                    ...
                    assets: [
                        {
                            id: 'fig1',
                            path: 'http://localhost:8080/api/v1/articles/54296/assets/elife-54296-fig1.tif',
                            type: 'image/jpeg',
                        },
                    ],
                    path: '/api/v1/articles/54296/export',
                    type: '',
                });
            });
    });
    ...
    it('Returns 404 for an invalid article manifest', async () => {
        return agent.get('/articles/00000/manifest').expect(404);
    });
});
...
describe('Get /article/id/export', () => {
        ...
        return agent
            ...
            .then((response) => {
                expect(response.text).toBe(
                    '<article><article-meta><contrib-group/><author-notes/><abstract><p/></abstract><abstract abstract-type="toc"><p/></abstract></article-meta><body><p/></body></article>',
                );
            });
    });
    ...
    it('exports an article with changes applied', async () => {
        ...
        return agent
            ...
            .then((response) => {
                expect(response.text).toBe(
                    '<article><article-meta><contrib-group/><author-notes/><abstract><p>Hello World!@123</p></abstract><abstract abstract-type="toc"><p/></abstract></article-meta><body><p/></body></article>',
                );
            });
    });
