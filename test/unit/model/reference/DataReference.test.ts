...
describe('DataReference', () => {
    ...
    describe('fromXml', () => {
        ...
        it('has specificUse property only with given specific-use attrib values', () => {
            ...
            expect(new DataReference(xmlWrapper.querySelector('element-citation') as Element).specificUse).toBe(
                'analyzed',
            );
            const xmlWrapper2 = parseXML(`<article><element-citation specific-use="generated"/></article>`);
            expect(new DataReference(xmlWrapper2.querySelector('element-citation') as Element).specificUse).toBe(
                'generated',
            );
            const xmlWrapper3 = parseXML(`<article><element-citation specific-use="foo"/></article>`);
            expect(
                new DataReference(xmlWrapper3.querySelector('element-citation') as Element).specificUse,
            ).toBeUndefined();
        });
        ...
        it('sets extLink to pub-id[pub-id-type="accession"] xlink:href content if ext-link not defined ', () => {
            const xmlWrapper = parseXML(
                `<article><element-citation><pub-id pub-id-type="accession" xlink:href="someURL">00000</pub-id></element-citation></article>`,
            );
        ...
        it('leaves extLink empty if ext-link and pub-id[pub-id-type="accession"] xlink:href are not defined', () => {
            const xmlWrapper = parseXML(
                `<article><element-citation><pub-id pub-id-type="accession">00000</pub-id></element-citation></article>`,
            );
            expect(new DataReference(xmlWrapper.querySelector('element-citation') as Element).extLink).toBe('');
        });
    });
    ...
    describe('toXml', () => {
        ...
        it('should exclude accession id if empty', () => {
            ...
            const xmlString = xmlSerializer.serializeToString(reference.toXml());
            expect(xmlString).toBe(
                '<element-citation publication-type="data" specific-use="analyzed"><year iso-8601-date="2014">2014</year><ext-link ext-link-type="uri" xlink:href="http://www.ncbi.nlm.nih.gov/nuccore/120407038">http://www.ncbi.nlm.nih.gov/nuccore/120407038</ext-link><pub-id pub-id-type="doi" xlink:href="00000">00000</pub-id><version>NM_009324.2</version></element-citation>',
            );
        });
    });
});
