...
describe('ReportReference', () => {
    ...
    describe('toXml', () => {
        ...
        it('should serialize a populated report reference', () => {
            const reference = new ReportReference({
                ...populatedReportRefJSON,
                articleTitle: {
                    doc: {
                        content: [
            ...
            const xmlString = xmlSerializer.serializeToString(reference.toXml());
            expect(xmlString).toBe(
                '<element-citation publication-type="report"><year iso-8601-date="year">year</year><ext-link ext-link-type="uri" xlink:href="extLink">extLink</ext-link><source>I am source text</source><publisher-name>publisherName</publisher-name><publisher-loc>publisherLocation</publisher-loc><volume>volume</volume><pub-id pub-id-type="doi">DOI</pub-id><pub-id pub-id-type="pmid">pmid</pub-id><pub-id pub-id-type="isbn">isbn</pub-id></element-citation>',
            );
        });
    });
});
