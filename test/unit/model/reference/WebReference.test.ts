...
describe('WebReference', () => {
  ...
  describe('toXml', () => {
    ...
    it('should serialize a empty web reference', () => {
      ...
      expect(xmlString).toBe('<element-citation><year iso-8601-date=""></year><ext-link ext-link-type="uri" xlink:href=""></ext-link><article-title/><source/></element-citation>');
    });
    ...
    it('should serialize a populated web reference', () => {
      ...
      const xmlString = xmlSerializer.serializeToString(reference.toXml());
      expect(xmlString)
        .toBe(`<element-citation><year iso-8601-date="year">year</year><ext-link ext-link-type="uri" xlink:href="extLink">extLink</ext-link><article-title>I am articleTitle text</article-title><source>I am source text</source><date-in-citation iso-8601-date="2011-10-10">October 10, 2011</date-in-citation></element-citation>`);
    });
    ...
    it('should not include date-in-citation if the value is empty', () => {
      ...
      const xmlString = xmlSerializer.serializeToString(reference.toXml());
      expect(xmlString)
        .toBe('<element-citation><year iso-8601-date="year">year</year><ext-link ext-link-type="uri" xlink:href="extLink">extLink</ext-link><article-title/><source/></element-citation>');
    });
  });
});
