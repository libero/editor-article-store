...
const mockGetByQuery = jest.fn(async () => ({
    assets: [{ _id: 'someAssetId123', articleId: 'articleId123', assetId: 'assetId123', fileName: 'someFile123.tiff' }],
    total: 1,
}));
...
describe('assetService', () => {
    ...
    describe('saveAsset', () => {
        it('saves a non tiff asset to the S3 bucket', async () => {
            ...
            expect(putObjectMock).toBeCalledWith(
        ...
        it('throws correct error if putObject fails', async () => {
            ...
            await expect(
                ...
                'Error when storing S3 object: { Key: 11111/11111111-1111-1111-1111-111111111111/someFileName.jpeg, Bucket: editorS3Bucket } - Some Error',
            );
        });
        ...
        it('throws correct error if tif conversion fails', async () => {
            (imageConverter as jest.Mock).mockImplementation(() => {
            ...
            await expect(
                ...
                'Error when converting .tif file: { Key: 11111/11111111-1111-1111-1111-111111111111/someFileName.tiff, Bucket: editorS3Bucket } - Error converting',
            );
        });
        ...
        it('throws correct error if storing the converted tif file fails', async () => {
            const mockPutObject = jest.fn((params) => {
                if (params.Key === '11111/11111111-1111-1111-1111-111111111111/someFileName.jpeg') {
                    throw new Error('Some Error');
                }
                return { promise: () => {} };
            });
            ...
            await expect(
                ...
                'Error when storing S3 object: { Key: 11111/11111111-1111-1111-1111-111111111111/someFileName.jpeg, Bucket: editorS3Bucket } converted from .tif file: { Key: 11111/11111111-1111-1111-1111-111111111111/someFileName.tif, Bucket: editorS3Bucket } - Some Error',
            );
        });
    });
