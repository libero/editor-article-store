...
describe('ArticleParser', function () {
    it('should parse article XML', () => {
        ...
        expect(manuscript.title.doc.textContent).toBe(
            `Example eLife Insight2 article with a complete2 reference 'front' section`,
        );

        expect(manuscript.abstract).toBeInstanceOf(EditorState);
        expect(manuscript.abstract.doc.textContent).toBe(
            `New studies examine how the different sub-structures in the sed nisi neque2, ornare id enim1 et, dapibus rutrum urna.`,
        );

        expect(manuscript.impactStatement).toBeInstanceOf(EditorState);
        expect(manuscript.impactStatement.doc.textContent).toBe(
            `New studies examine how the different sub-structures in the sed nisi neque2, ornare id enim1 et, dapibus rutrum urna.`,
        );

        expect(manuscript.acknowledgements).toBeInstanceOf(EditorState);
        expect(manuscript.acknowledgements.doc.textContent).toBe(
            `Acknowledgements section containing only paragraph type elements (no block content such as tables, figures and so on).`,
        );

        expect(manuscript.body).toBeInstanceOf(EditorState);
        expect(manuscript.body.doc.textContent).toMatchSnapshot();

        expect(manuscript.keywordGroups).toEqual(
        ...
        expect(get(manuscript, 'keywordGroups.author-keywords.keywords.1.content.doc.textContent')).toBe(
            'climbing fiber',
        );
        expect(get(manuscript, 'keywordGroups.author-keywords.keywords.2.content.doc.textContent')).toBe(
            'reinforcement learning',
        );
        expect(get(manuscript, 'keywordGroups.author-keywords.keywords.3.content.doc.textContent')).toBe('vitae');
        expect(get(manuscript, 'keywordGroups.author-keywords.keywords.4.content.doc.textContent')).toBe(
            'Purkinje cells',
        );
        ...
        expect(manuscript.authors[1]).toStrictEqual(
            expect.objectContaining({
                _id: 'unique_id',
                firstName: 'Jeanine',
                lastName: 'Smith',
        ...
        expect(manuscript.authors[2]).toStrictEqual(
            expect.objectContaining({
                _id: 'unique_id',
                firstName: 'Jack',
                lastName: 'London',
        ...
        expect(manuscript.authors[3]).toStrictEqual(
            expect.objectContaining({
                _id: 'unique_id',
                firstName: 'Mark',
                lastName: 'Twain',
        ...
        expect(manuscript.authors[4]).toStrictEqual(
            expect.objectContaining({
                _id: 'unique_id',
                firstName: 'Alexandr',
        ...
        expect(get(manuscript, 'authors.0.bio.doc.textContent')).toBe(
            'Fred Atherden is in the Production Department, eLife Sciences, Cambridge, United Kingdoms Creative Commons Attribution License',
        );
        expect(get(manuscript, 'authors.1.bio.doc.textContent')).toBe(
            'Jeanine Smith III is in the Department, University, City, Country',
        );
        expect(get(manuscript, 'authors.2.bio.doc.textContent')).toBe(
            'Jeanine Smith III is in the Department, University, City, Country',
        );
        expect(get(manuscript, 'authors.3.bio.doc.textContent')).toBe(
            'Jeanine Smith III is in the Department, University, City, Country',
        );
